...
void IspisVozaca(Autobus Podaci, int n, int br){
	if(br==1){
		cout<<endl<<setfill('-')<<setw(108)<<"-"<<endl<<setfill(' ');
		cout<<setw(4)<<left<<" Br"<<setw(13)<<"|"<<setw(20)<<"Relacija"<<setw(9)<<"|"<<setw(16)<<"Polazak"<<setw(9)<<"|"<<setw(16)<<"Povratak"<<setw(7)<<"|"<<setw(13)<<"Agencija"<<"|"<<endl;
		cout<<setfill('-')<<setw(108)<<"-"<<endl<<setfill(' ');	
	}
	cout << " "<<br <<setw(2)<< "." << setw(3)<<"|"<< setw(13)<<right<<Podaci.PolazakMjesto<<" - "<<setw(14)<<left<<Podaci.PovratakMjesto<< setw(3)<<"|"<<setfill('0')<<setw(2)<<right<<Podaci.DatumPolaska.dan<<"."<<setfill('0')<<setw(2)<<right<<Podaci.DatumPolaska.mjesec<<"."<<Podaci.DatumPolaska.godina<<".  " << "(" <<setfill('0')<<setw(2)<< Podaci.PolazakVrijeme.sat<<":"<<setfill('0')<<setw(2)<<Podaci.PolazakVrijeme.minuta;
	cout << ")"<<setfill(' ')<<setw(5)<<left<<"  |" << setfill('0')<<setw(2)<<Podaci.DatumPovratka.dan<<"."<<setfill('0')<<setw(2)<<Podaci.DatumPovratka.mjesec<<"."<<Podaci.DatumPovratka.godina<<".  " << "(" <<setfill('0')<<setw(2)<<Podaci.PovratakVrijeme.sat<<":"<<setfill('0')<<setw(2)<<Podaci.PovratakVrijeme.minuta<< ")  "<<setfill(' ')<<setw(6)<<"|"<<setw(9)<<right<< Podaci.Agencija<<setw(5)<<"|"<<endl<<left;	
}

void PretragaVozaca (Autobus Podaci[], string ime, string prezime, int i, int n, int br){
	if (i==n){
		if (br==0){
			cout << "\n\tUneseni vozac ne postoji ili nema zakazane voznje.\n" << endl;
		}
	}else if((Podaci[i].ImeVozaca) == ime && (Podaci[i].PrezimeVozaca == prezime)){
		br++;
		IspisVozaca(Podaci[i], n, br);
	...
	if(PrviMeni == 1){ // 1.1. Ispis svih gradskih linija
		...
		if(TreciMeni == 1){ // 2.1. Rezervacije
			...
			if(Pretrazivanje == 1){ // 2.2.1 Pretrga po imenu agencije
				...
				cout<<setfill('-')<<setw(108)<<"-"<<endl<<setfill(' ')<<endl;
				system("pause");
				system("cls");
				Meni();
			}			
